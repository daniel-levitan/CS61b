...
//        int max = 128001;
        ...
        for (int N = 1000; N < max; N = N * 2) {
            Ns.add(N);
//            opCounts.add(ops);
            opCounts.add(N);
            Stopwatch sw = new Stopwatch();
//            for (int j = 0; j < ops; j++) {
                // This is the experiment. Instantiate a list and add N elements to it
                AList<Integer> my_list = new AList();
                for (int i = 0; i < N; i++)
                    my_list.addLast(1);
//            }
            times.add(sw.elapsedTime());
    ...
    public static TimingData timeSLListGetLast() {
        ...
        for (int N = 1000; N < max; N = N * 2) {
            ...
            // Setting the list to N elements
            SLList<Integer> my_list = new SLList<>();
            for (int i = 0; i < N; i++) my_list.addLast(1);

            Stopwatch sw = new Stopwatch();
            for (int op = 0; op < ops; op++) {
//                for (int i = 0; i < 10000; i++) my_list.getLast();
                my_list.getLast();
            }
            times.add(sw.elapsedTime());
